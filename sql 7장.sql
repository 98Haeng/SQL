...
SELECT @myVar4 , Name FROM usertbl WHERE height > 180 ; -- 변수를 이용하여 결과를 보기좋게 출력

SET @myVar1 = 3 ;
PREPARE myQuery 
    FROM 'SELECT Name, height FROM usertbl ORDER BY height LIMIT ?'; -- Limit에는 변수를 사용할 수 없음
EXECUTE myQuery USING @myVar1 ;
-- </실습 1> --

USE sqldb ;
SELECT AVG(amount) AS '평균 구매 개수' FROM buytbl ;
SELECT CAST(AVG(amount) AS SIGNED INTEGER) AS '평균 구매 개수'  FROM buytbl ; -- avg(count)를 정수로 변환하기 위해 cast, convert 사용, signed integer : 정수
-- 또는
...
-- CONCAT을 이용하여 숫자, 문자 연결하여 곱셈식을 만들기
...
SELECT 1 > '2mega'; -- 정수인 2로 변환되어서 비교, 거짓은 0으로, 참은 1로 표현
...
SELECT ASCII('A'), CHAR(65); -- 결과가 BLOB로 뜨면 마우스 오른쪽 버튼 클릭 후 Open value in viewer 선택 후 text 클릭하면 정상적으로 나옴

SELECT BIT_LENGTH('abc'),  CHAR_LENGTH('abc'), LENGTH('abc'); -- 비트수, 글자개수, 바이트개수
...
SELECT movie_film FROM movietbl  WHERE movie_id=3 
	INTO DUMPFILE 'C:/SQL/Movies/Mohican_out.mp4'; -- 영화 동영상 다운로드
...
SELECT JSON_VALID(@json) AS JSON_VALID; -- 형식에 이상이 없는지 확인 이상 없으면 1 있으면 0
SELECT JSON_SEARCH(@json, 'one', '성시경') AS JSON_SEARCH; -- one : 첫번째 만나는 하나만 찾음, all : 맞는 모두를 찾음
SELECT JSON_EXTRACT(@json, '$.usertbl[2].name') AS JSON_EXTRACT; -- 위치를 지정해주면 위치에 있는 값을 찾음
SELECT JSON_INSERT(@json, '$.usertbl[0].mDate', '2009-09-09') AS JSON_INSERT; -- json에 mdate 입력
SELECT JSON_REPLACE(@json, '$.usertbl[0].name', '홍길동') AS JSON_REPLACE; -- 데이터값 바꿈
SELECT JSON_REMOVE(@json, '$.usertbl[0]') AS JSON_REMOVE; -- 데이터값 제거
